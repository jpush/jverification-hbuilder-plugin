...
- (void)parseOption:(NSDictionary *)option {
    ...
    if (option[@"position"]) {
        _position = [DCUniConvert NSString:option[@"position"]];
    }
    
    if (option[@"title"]) {
        _title = [DCUniConvert NSString:option[@"title"]];
    }
    
    if (option[@"titleAlign"]) {
        NSString *titleAlign = [DCUniConvert NSString:option[@"titleAlign"]];
    ...
    if (option[@"titleColor"]) {
        _titleColor = [DCUniConvert UIColor:option[@"titleColor"]];
    }
    
    if (option[@"titleFontSize"]) {
        _titleFontSize = [DCUniConvert CGFloat:option[@"titleFontSize"]] / 2.0;
    }
    
    if (option[@"content"]) {
        _content = [DCUniConvert NSString:option[@"content"]];
    }
    
    if (option[@"contentAlign"]) {
        NSString *contentAlign = [DCUniConvert NSString:option[@"contentAlign"]];
    ...
    if (option[@"contentColor"]) {
        _contentColor = [DCUniConvert UIColor:option[@"contentColor"]];
    }
    
    if (option[@"contentFontSize"]) {
        _contentFontSize = [DCUniConvert CGFloat:option[@"contentFontSize"]] / 2.0;
    }
    ...
    if (option[@"buttons"]) {
        if ([option[@"buttons"] isKindOfClass:[NSArray class]]) {
            ...
            NSLog(@"buttons 参数错误 %@",option[@"buttons"]);
        }
    }
    ...
    if (option[@"checkBox"]) {
        if ([option[@"checkBox"] isKindOfClass:[NSDictionary class]]) {
            ...
            NSLog(@"checkBox 参数错误 %@",option[@"checkBox"]);
        }
    }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (_checkBox) {
        ...
        NSString *title = _checkBox[@"title"] ? [DCUniConvert NSString:_checkBox[@"title"]] : @"";
        UIColor *titleColor = _checkBox[@"titleColor"] ? [DCUniConvert UIColor:_checkBox[@"titleColor"]] : [UIColor blackColor];
        _isSelected = [DCUniConvert BOOL:_checkBox[@"isSelected"]];
        ...
        checkBoxView.onTintColor = [DCUniConvert UIColor:@"#108EE9"];
        checkBoxView.onCheckColor = [UIColor whiteColor];
        checkBoxView.onFillColor = [DCUniConvert UIColor:@"#108EE9"];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    for (int i = 0; i < _buttons.count; i++) {
        ...
        NSString *title = item[@"title"] ? [DCUniConvert NSString:item[@"title"]] : @"";
        UIColor *titleColor = item[@"titleColor"] ? [DCUniConvert UIColor:item[@"titleColor"]] : [UIColor blackColor];
        ...
        if (i == 0) {
            ...
            line.backgroundColor = [DCUniConvert UIColor:kDCAlertViewDefaultLineColor];
            [btn addSubview:line];
        }
